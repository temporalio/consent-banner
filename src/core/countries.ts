// Narrow, literal-typed source: drives the `CountryCode` union below.
...
// Widened, mutable export for consumers (the Combobox component is generic
// over its option-object type and rejects a `readonly` literal tuple).
export const countriesWithCodes: { code: string; label: string }[] =
  COUNTRIES_DATA.map((c) => ({ code: c.code, label: c.label }));
