...
// The regime endpoint is configurable so hosts without a same-origin `/api/geo`
// route (docs / learn / pages, or a future consent.temporal.io) can point the
// shared banner at the canonical endpoint via the `geo-endpoint` attribute.
describe('TemporalCookieBanner geo-endpoint', () => {
  ...
  it('defaults to the same-origin /api/geo route', async () => {
  ...
  it('calls the absolute endpoint set via the geo-endpoint attribute', async () => {
    const fetchMock = captureGeoFetch();
    const el = document.createElement(ELEMENT_NAME) as TemporalCookieBanner;
    el.setAttribute('geo-endpoint', 'https://consent.temporal.io/api/geo');
    document.body.append(el);
    await tick();
    await tick();
    await el.updateComplete;
